...
class FeatureExtractionService:
    def __init__(self, config_path: str):
        ...
        ckpt_path = os.path.join(
            cfg['training']['model_dir'], 'ckpts/best.ckpt')
    ...
    def extract_feature(self, frames_path: str) -> torch.Tensor:
        ...
        img_path_list = [x for x in sorted(
            os.listdir(frames_path)) if x.endswith(".png")]
        custom_data = [{
            'name': f"custom-data@{frames_path}",
            'num_frames': len(img_path_list),
        }]

        custom_batch = collate_fn_(
            custom_data, cfg["data"], cfg["task"], is_train=False, gloss_tokenizer=model.gloss_tokenizer)
        # print(custom_batch)
            ...
            for ii in range(len(custom_batch['name'])):
                ...
                for suffix, output_data in suffix2data.items():
                    ...
                    if suffix == 'inputs_embeds':
                        valid_len = torch.sum(
                            forward_output['transformer_inputs']['attention_mask'][ii])
                        output_data.append({**entry,
...
class PredictionService:
    ...
    def predict(self, frames_path: str, features: torch.Tensor) -> (str, str):
        cfg = self.cfg
        model = self.model
        ...
        custom_batch = collate_fn_(custom_data, cfg['data'], cfg['task'], False,
                                   gloss_tokenizer=model.gloss_tokenizer, text_tokenizer=model.text_tokenizer)
        ...
        with torch.no_grad():
            ...
            for k, gls_logits in forward_output.items():
                ...
                if logits_name in ['rgb_', 'keypoint_', 'fuse_', 'ensemble_last_', 'ensemble_early_', '']:
                    ...
                    batch_pred_gls = model.gloss_tokenizer.convert_ids_to_tokens(
                        ctc_decode_output)
            ...
            if 'aux_logits' in forward_output:
                # ['rgb', 'keypoint]
                for stream, logits_list in forward_output['aux_logits'].items():
                    # might be empty
                    lengths_list = forward_output['aux_lengths'][stream]
        ...
        print(results)
        return results[f"custom-data@{frames_path}"]['txt_hyp'], results[f"custom-data@{frames_path}"]['gls_hyp']
