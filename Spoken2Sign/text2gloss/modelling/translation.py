...
from .Tokenizer import GlossTokenizer_G2T, GlossTokenizer_G2G, GlossTokenizer_as_mbart, TextTokenizer
import pickle, math
...
class TranslationNetwork(torch.nn.Module):
    def __init__(self, input_type, cfg, task) -> None:
        ...
        assert self.input_type in ['gloss','feature','gt_gloss+feature','pred_gloss+feature','text']
        ...
        if self.task == 'S2T_glsfree': 
            ...
            # self.gloss_tokenizer = GlossTokenizer_as_mbart(tokenizer_cfg=cfg['GlossTokenizer'])
    ...
    def generate(self, 
        ...
        if self.task == 'T2G':
            output_dict['decoded_sequences'] = self.gloss_tokenizer.batch_decode(output_dict['sequences'])
        else:
            output_dict['decoded_sequences'] = self.text_tokenizer.batch_decode(output_dict['sequences'])
        # print('op_Sequence: ', output_dict['sequences'])
